...
class PlaygroundRequest(BaseModel):
    ...
    temperature_range: Optional[ParameterRange] = None
    top_p_range: Optional[ParameterRange] = None
    top_k_range: Optional[ParameterRange] = None
    max_tokens_range: Optional[ParameterRange] = None
    # Fixed parameters (if ranges not provided, use these)
    temperature: Optional[float] = 0.7
    top_p: Optional[float] = 1.0
...
@router.post("/playground/generate-batch")
async def playground_generate_batch(request: PlaygroundRequest):
    """
    ...
    for i in range(request.num_responses):
        ...
        # Handle top_k
        if request.top_k_range:
            step_size = (request.top_k_range.max - request.top_k_range.min) / max(1, request.num_responses - 1)
            params["top_k"] = int(request.top_k_range.min + (i * step_size))
